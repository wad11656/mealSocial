<template>
  <div class="recipePage">
    <h1 class="title">My Recipes <button class="addButton" v-on:click="addRecipe()">New Recipe </button></h1>
    <div class = "recipeCard" v-for="recipe in recipeList" :key="recipe.id" v-on:click="editRecipe(recipe)"> 
...
export default {
    ...
    methods: {
        ...
        addIngredient() {
          let ojbTemp = {"name" : this.ingredientName, "measurement" : this.measurement };
        ...
        async getOneIngredientData(id) {
          IngredientService.getOneIngredient(id).then((ingredientList =>{
            this.$set(this, "ingredientList", JSON.parse(ingredientList.ingredientList));
          }).bind(this));
        },
